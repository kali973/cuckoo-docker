...
def create_window():
    if os.name == 'posix':
        ...
        output_text = tk.Text(window, width=80, height=30)
        output_text.pack(pady=10)

        close_button = tk.Button(window, text="Fermer", command=lambda: close_window(window))
        close_button.pack(pady=5)

        build_button = tk.Button(window, text="Exécuter build.py", command=lambda: run_build_button(output_text, close_button))
