...
class Droidmon(Processing):
    ...
    def _handle_java_net_URL_openConnection(self, api_call):
        ...
        json = {}
        if api_call["result"] != "":
            json["request"] = api_call["result"]["request_method"] + " " + api_call["this"] + " " + api_call["result"][
                "version"]
            json["response"] = api_call["result"]["version"] + " " + str(api_call["result"]["response_code"]) + " " + \
                               api_call["result"]["response_message"]
        else:
