...
def submit_tasks(target, options, package, custom, owner, timeout, priority,
    ...
    if is_baseline:
        if remote:
            print
            "Remote baseline support has not yet been implemented."
            return
    ...
    if is_url and is_unique:
        print
        "URL doesn't have --unique support yet."
        return
