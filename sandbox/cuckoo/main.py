...
@main.command()
@click.pass_context
@click.option("--conf", type=click.Path(exists=True, file_okay=True, readable=True),
              help="Flat key/value configuration file")
def init(ctx, conf):
...
@click.option("--file", "--filepath", type=click.Path(exists=True),
              help="Specify a local copy of a community .tar.gz file")
@click.pass_context
...
def submit(ctx, target, url, options, package, custom, owner, timeout,
    ...
    try:
        ...
        for category, target, task_id in l:
            if task_id:
                print
                "%s: %s \"%s\" added as task with ID #%s" % (
                    bold(green("Success")), category, target, task_id
                )
            else:
                print
                "%s: %s \"%s\" as it has already been analyzed" % (
...
@click.option("--service", type=click.Path(exists=True), default="/usr/sbin/service",
              help="Path to service(8) for invoking OpenVPN")
...
def migrate(revision):
    ...
    if not migrate_database(revision):
        print
        red(">>> Error migrating your database..")
        exit(1)

    print
    yellow(">>> Your database migration was successful!")


@main.command("import")
@click.option("--copy", "mode", flag_value="copy", default=True,
              help="Copy all existing analyses to the new CWD (default)")
...
def import_(ctx, mode, path):
    ...
    print
    yellow("You are importing an existing Cuckoo setup. Please")
    print
    yellow("understand that, depending on the mode taken, if ")
    print
    yellow("you remove the old Cuckoo setup after this import ")
    print
    yellow("you may still"), red("lose ALL of your data!")
    print
    print
    yellow("Additionally, database migrations will be performed ")
    print
    yellow("in-place*. You won't be able to use your old Cuckoo ")
    print
    yellow("setup anymore afterwards! However, we'll provide ")
    print
    yellow("you with the option to create a SQL backup beforehand.")
    print
    print
    red("TL;DR Cleaning the old setup after the import may")
    print
    red("corrupt your new setup: its SQL, MongoDB, and ")
    print
    red("ElasticSearch database may be dropped and, in 'symlink'")
    print
    red("mode, the analyses removed.")
    print
    print
    yellow("*: Except for sqlite3 databases in combination with")
    print
    yellow("   the import 'copy' approach.")
    print
...
def server(ctx, host, port, uwsgi, nginx):
    ...
    if uwsgi:
        ...
        if os.environ.get("VIRTUAL_ENV"):
            ...
            "virtualenv =", os.environ["VIRTUAL_ENV"]
        print
        "module = cuckoo.apps.distributed"
        print
        "callable = app"
        print
        "uid =", username
